...
window.addEventListener("load", function () {
  ...
  const intervalo = this.setTimeout(function () {
    const pregunta = confirm("Queres conocer mas musica?");
    const aceptar = document.querySelector(".perfil");
    const agregado = `<p style = "color:green" id = "textoVerde"><a>Usuario oficial de Spotifront</a></p>`;
    ...
    if(pregunta){
      ...
      aceptar.innerHTML = `<div class="perfil">
      <h5>PERFIL</h5>
      ${agregado}
      ...
      const textoAg = document.querySelector("#textoVerde");

      textoAg.addEventListener("click", function(){
